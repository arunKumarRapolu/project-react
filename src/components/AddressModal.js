...
class AddressModal extends Component {
    ...
    saveAddress = async(e) => {
      ...
      if(this.state.mobile == null || this.state.mobile == '' || !this.state.mobile.match(phonenoreg)){
      ...
      if(this.state.pin == null || this.state.pin == '' || !this.state.pin.match(pincodereg)){
      ...
      if(isProceed){
        ...
        let saved = await this.props.saveAddress(address);
        if(saved == "success"){
    ...
    render(){
        ...
        const renderStateOptions = allStates.map((val,key) => {
            return(
            <option value={val} key={key}>{val} - India</option>
            )
        })
        return(
...
export default compose(withRouter,connect(mapState, actionCreators))(AddressModal);